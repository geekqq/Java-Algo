...
public class TreeClassDemo {
    ...
    public static Tree solve() {
        ...
        if (n == 1) {
            ...
            for (int i = 0; i < n - 1; i++) {
                int u = sc.nextInt();
                int v = sc.nextInt();
                Set<Integer> uEdges = nodeMap.get(u);
                if (uEdges == null) {
                    uEdges = new HashSet<>();
                }
                uEdges.add(v);
                nodeMap.put(u, uEdges);
                Set<Integer> vEdges = nodeMap.get(v);
                if (vEdges == null) {
                    vEdges = new HashSet<>();
                }
                vEdges.add(u);
            ...
            for (int nodeId : nodeMap.get(1)) {
                nodeMap.get(nodeId).remove(1);
    ...
    private static void createEdge(Tree parent, int nodeId) {
        Set<Integer> nodeEdges = nodeMap.get(nodeId);
        ...
        if (hasChild) {
            ...
            for (int neighborId : nodeEdges) {
                nodeMap.get(neighborId).remove(nodeId);
